...
<div id="fl-advanced-form" class="fl-settings-form">
	...
	<p><?php _e( 'Fine-tune the builder behavior with these advanced options.', 'fl-builder' ); ?></p>
	...
	<form id="advanced-form" action="<?php FLBuilderAdminSettings::render_form_action( 'advanced' ); ?>" method="post">
		...
		foreach ( $groups as $group_key => $group ) {
			printf( '<div class="advanced-group fl-toggle-group"><h3 class="fl-toggle-group-header">%s<span class="dashicons dashicons-arrow-down-alt2"></span></h3><div class="advanced-group-options">', $group['label'] );
			foreach ( $settings as $key => $setting ) {
				...
				if ( isset( $setting['type'] ) && 'text' == $setting['type'] ) {
					...
					printf( '<div class="advanced-option"><span class="title">%s%s%s</span><label class="fl-toggle-switch" for="%s"><input type="checkbox" id="%s" name="%s" value="%s"%s%s><span></span></label></div>', $setting['label'], $description, $link, $key, $key, $key, $setting['default'], $checked, $hasdepend );
				}
			}
			echo '</div></div>';
		}
